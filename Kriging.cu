...
__global__ void nodiag_normalize(REAL A_semivariance_matrix[][D_SIZE], REAL inversed_A[][D_SIZE], int n, int i)
{
    ...
    if (x < n && y < n)
        if (x == i && x != y)
        {
            inversed_A[x][y] /= A_semivariance_matrix[i][i];
            A_semivariance_matrix[x][y] /= A_semivariance_matrix[i][i];
        }
}
// diagonal_normalize
...
int main()
{
    ...
    string result_aoas, result_soa;
    cout << "save the result_aoas in: ";
    cin >> result_aoas;
    cout << "save the result_soa in: ";
    cin >> result_soa;
    /****/
    ofstream fout_base_AOAS(result_root + result_aoas);
    if (!fout_base_AOAS)
    {
        cout << "\nCannot Save File ! " << result_aoas << endl;
        exit(1);
    }
    ...
    ofstream fout_base_SOA(result_root + result_soa);
    if (!fout_base_SOA)
    {
        cout << "\nCannot Save File ! " << result_soa << endl;
        exit(1);
    }
